; Exercise 2.19. Consider the change-counting program of section 1.2.2. It would
; be nice to be able to easily change the currenty used by the program, so that
; we could compute the number of ways to change a British pound, for example. As
...
; be nicer to be able to supply a list of coins to be used for makking change.
;
...
; These are really simple, getting the first object of a list is just taking the
; car of it
(define (first-denomination coin-values)
  (car coin-values))
; Getting everything but the first object of a list is just the cdr
(define (except-first-denomination coin-values)
  (cdr coin-values))
; Seeing if there is anything left in the list is just checking to see if it is
; null or not
