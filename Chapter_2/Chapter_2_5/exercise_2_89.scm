...
; solutions here are mostly adapted (some functions identically copied)
; from http://jots-jottings.blogspot.com/2012/04/sicp-exercise-289-representing-dense.html
...
(define (install-polynomial-package)
  ...
  ; keeping make-term compatible for sparse representations as well, we just
  ; allow dense representations to ignore the order term
  ...
  ; for dense representations we only need to return the term
  (define (coeff term) term)
  (define (same-variable? v1 v2)
    (and (variable? v1) (variable? v2) (eq? v1 v2)))
  ; since adjoin term now takes the order of the term as an explicit 
  ; argument we need to update add-terms
  (define (add-terms L1 L2)
    (cond ((empty-termlist? L1) L2)
          ((empty-termlist? L2) L1)
          (else
            ; instead of calculating order each time we need it, its easier to
            ; calculate it once under the let call. Remember that order now
            ; takes the full list, instead of the term itself
                    ...
                    (else
                     (adjoin-term
                       ...
                       (add-terms (rest-terms L1)
                                  (rest-terms L2)))))))))
  ; mul-term-by-all-terms needs updated to pass the order just like adjoin-term
  ; did
  (define (mul-terms L1 L2)
    (if (empty-termlist? L1)
      (the-empty-termlist)
      (add-terms (mult-term-by-all-terms (order L1) (coeff (first-term L1)) L2)
                 (mul-terms (rest-terms L1) L2))))

  ; in mult-term-by-all-terms we need to know the order of a term
  ; again. Therefore we will need to update the order operation to take
  ; a term list instead working on just a term.
  (define (mult-term-by-all-terms o1 c1 L)
    (if (empty-termlist? L)
      (the-empty-termlist)
      (let ((t2 (first-term L))
            (this-order (+ o1 (order L))))
        (adjoin-term
          this-order
          (mul c1 (coeff t2))
          (mult-term-by-all-terms o1 c1 (rest-terms L))))))
  ...
  (define (=zero?-termlist L)
    (cond ((empty-termlist? L)
          ...
          #t)
          (else #f)))
  ; negate the term list
  (define (negate-terms L)
    (if (empty-termlist? L)
        (the-empty-termlist)
        ; had to update to work with new adjoin-term
        (adjoin-term (order L)
...
;tests
(define poly1 (make-polynomial 'x (list (make-integer 4)
                                        (make-integer 2)
                                        (make-integer 1)
                                        (make-integer 5))))

(define poly2 (make-polynomial 'x (list (make-integer 7)
                                        (make-integer 2)
                                        (make-integer 2)
                                        (make-integer 1))))

(define poly3 (make-polynomial 'x (list (make-integer 0)
                                        (make-integer 0)
                                        (make-integer 0)
                                        (make-integer 0))))

(define poly4 (make-polynomial 'x 
                               (list  (make-complex-from-real-imag 
                                                  (make-integer 1) (make-integer 0)))))

(define poly5 (make-polynomial 'x (list (make-integer 4)
                                        (make-real 2)
                                        (make-rational 1 2)
                                        (make-complex-from-real-imag (make-integer 1) 
                                                                     (make-integer 0)))))
