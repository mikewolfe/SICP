...
;(for scheme-number arugments). You can now easily make the rational-arithmetic
;package reduce fracitons to lowest terms by having make-rat call reduce
;before combining the given numerator and denominator to form a rational
;number. The system now handles rational expressions in either integers or
;polynomials. To test your program, try the example at the beginning of this
;extended exercise:
;
...
;; COPY COMPLETE SYSTEM FROM 2.95 INCLUDING COMMENTS AND TESTS
...
; generic reduce added for dealing with integers and polynomials
(define (reduce a b)
...
(define (install-integer-package)
  ...
  (put 'reduce '(integer integer)
       (lambda (x y) (let ((vals (reduce-integers x y)))
                       (list (tag (car vals)) (tag (cadr vals))))))
  'done)
...
(define (install-rational-package)
  ...
  (define (make-rat n d)
    ...
    (cond ((integer? n) (make-rat (make-integer n) d))
          ...
          ((and (valid-component? n) (valid-component? d)) 
           (let ((numerdenom (reduce n d)))
                 (cons (car numerdenom) (cadr numerdenom))))
          (else (error
...
(define (install-polynomial-package)
  ...
  ; define a new procedure to keep everything in integers
  (define (pseudoremainder-terms L1 L2)
    (let* ((t1 (first-term L1))
           (t2 (first-term L2))
           (intfact (exp (coeff t2) (make-integer (+ 1 (order t1)
                                                     (- (order t2))))))
           (psuedoL1 (mul-term-by-all-terms (make-term 0 intfact)
                                            L1)))
    ; just pull out the remainder from div-terms
    (cadr (div-terms psuedoL1 L2))))
  ; for part b I am following along with
  ...
  ; find the gcd for every term in the term list, by iterating through the list
  ; and finding the greatest-common-divisor among everything
  (define (find-gcd c L)
    (if (empty-termlist? L)
        c
        (find-gcd (greatest-common-divisor c (coeff (first-term L)))
                  (rest-terms L))))
  ; divide everything by the found gcd
  ...
  ;; Exercise 2.97 additions
  (define (reduce-terms N D)
    ; reduce each term list by the same gcd
    (let ((g (gcd-terms N D)))
      (list (car (div-terms N g)) (car (div-terms D g)))))

  (define (reduce-poly N D)
    ; reduce two polynomials by the same gcd 
    (let ((thisvar (select-variable-from-polys N D))
          (reduced (reduce-terms (term-list N) (term-list D))))
          (list (make-poly thisvar (car reduced))
                (make-poly thisvar (cadr reduced)))))
  ...
  ; reduce polynomials by shared gcd
  (put 'reduce '(polynomial polynomial)
       (lambda (p1 p2) (let ((vals (coerce-and-call p1 p2 reduce-poly)))
                         ; must tag as polynomials once done
                         (list (tag (car vals)) 
                               (tag (cadr vals))))))
  'done)
...
(define dvr (greatest-common-divisor p1 p2))
dvr
; test that its actually a divisor, should have a remainder of zero and the
; quotients should be greater than the divisor
(div p1 dvr)
(div p2 dvr)

; Next test and see if you can get a gcd of the quotients
...
; now collapses down to the smallest integers possible
;
; Check to see that generic reduction is working
(reduce q1 q2)
(car (reduce q1 q2))
(cdr (reduce q1 q2))

(define v1 (reduce (make-integer 12) (make-integer 4)))
v1
(car v1)
(cadr v1)

; Check to see that it automatically reduces fractional polynomials to
; lowest terms
...
(define p4 (make-polynomial-from-coeffs 'x
                                        (list (make-integer 1)
                                              zero
                                              (make-integer -1))))
p1
...
rf1
rf2
; check to see if we get the correct answer:
(add rf1 rf2)
; And we do albeit every sign is flipped from the answer in the book, since its
; a rational fraction this is the same answer
